...
def mtz2tw(n,c,e,l):
    ...
    for i in range(2,n+1):
        model.addConstr(e[i] + quicksum(max(e[j]+c[j,i]-e[i],0) * x[j,i] for j in range(1,n+1) if i != j) \
                        <= u[i], "LiftedLB(%s)"%i)
...
def tsptw2(n,c,e,l):
    ...
    for j in range(2,n+1):
        model.addConstr(quicksum(u[i,j] + c[i,j]*x[i,j] for i in range(1,n+1) if i != j) -
